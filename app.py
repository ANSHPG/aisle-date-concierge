...
Decision rules:
1. If they ask for a simple suggestion, give a short and concise answer.
2. If they ask for a complete plan or itinerary, give a detailed structured answer.
3. Use weather only if it affects the plan. Use news only if safety, traffic, or events matter. Use restaurants only when relevant.

Planning rules:
- Prefer indoor plans during bad weather. Prefer outdoor options only when weather is comfortable.
- Avoid unsafe or crowded areas if news mentions them. Avoid unnecessary long travel.
- Match suggestions to budget, vibe, and convenience. Prioritize romance, comfort, practicality, realism.

Response style:
- Keep answers short by default (2-5 lines). Only expand when the user explicitly wants a full plan.
- Do not repeat tool outputs unnecessarily. Be direct, helpful, and natural.

If the user asks for a full plan, return in this format:
1. Best Date Plan (Complete timeline)
2. Best Restaurant Recommendation (Name + why)
3. Best Area / Location (Why this area)
4. Best Time (When and why)
5. Things To Avoid (Traffic, weather, safety, budget mistakes)
6. Backup Plan (Alternative option)

Tone: Warm, concise, romantic, practical, and personalized.
""")
...
def init_state():
    ...
    if "chat_log" not in st.session_state:
        st.session_state.chat_log = []  # list of dicts: {role, content}
...
def finalize_response():
    ...
    st.session_state.messages.append(AIMessage(content=final.content))
    st.session_state.chat_log.append({"role": "assistant", "content": final.content})
    st.session_state.pending_calls = []
    st.session_state.need_agent_step = False
    st.session_state.hops = 0
    st.session_state.thinking = False
...
for msg in st.session_state.chat_log:
    role = msg["role"]
    label = "You" if role == "user" else "Aisle"
    st.markdown(
        f'<div class="msg-row {role}"><div>'
        f'<div class="bubble-label" style="text-align:{"right" if role == "user" else "left"};">{label}</div>'
