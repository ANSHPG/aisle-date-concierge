...
.plan-card{
  background: linear-gradient(155deg, rgba(212,175,55,0.06), rgba(201,24,74,0.04));
  border: 1px solid var(--line);
  border-radius: 14px;
...
.plan-block{ margin: 0 0 0.9rem 0; padding-bottom: 0.85rem; border-bottom: 1px dashed var(--line); }
.plan-block:last-child{ margin-bottom: 0; padding-bottom: 0; border-bottom: none; }
.plan-label{
  font-family:'JetBrains Mono', monospace;
  font-size: 0.66rem;
  letter-spacing: 0.14em;
...
st.iframe(
    ...
    (function() {
        ...
        window.parent.document.addEventListener('click', function(e) {
            ...
            var text = '';
            if (targetId) {
                var src = window.parent.document.getElementById(targetId);
...
def get_ip_city():
    """Resolve an approximate city from the network's public IP address.

    Browser-side navigator.geolocation doesn't work reliably from inside
    Streamlit's sandboxed iframes (the permissions-policy for geolocation
    isn't delegated to them, so the request hangs instead of resolving).
    IP-based lookup needs no browser permission prompt at all, so it's the
    reliable option here. It's approximate (city-level, tied to your ISP's
    network, not GPS-precise).
    """
    try:
...
def _flatten_blank_lines(text: str) -> str:
    """Replace whitespace-only lines with a zero-width space.

    Streamlit's markdown renderer treats a run of raw HTML as a single HTML
    block, and per CommonMark that block ends at the first *blank* line —
    everything after gets parsed as ordinary markdown again, which is how
    multi-paragraph LLM text (which naturally contains blank lines between
    paragraphs) can leak out as stray visible text mid-widget. A real single
    newline is harmless and renders fine with our `white-space: pre-wrap`
    bubbles, so we only need to neutralize genuinely blank lines.
    """
    lines = text.split("\n")
...
def safe_inline(text):
    """Escape text for embedding as HTML *content* inside a tag."""
    text = str(text or "")
    text = html_lib.escape(text, quote=False)
    text = _flatten_blank_lines(text)
    return text


def safe_attr(text):
    """Escape text for embedding inside an HTML attribute value."""
    text = str(text or "")
...
def render_timeline_html(timeline):
    ...
    if isinstance(timeline, list):
        for step in timeline:
            ...
            if not a:
                continue
            t, a = safe_inline(t), safe_inline(a)
        ...
        if len(pieces) > 1:
            ...
            for piece in pieces:
                ...
                if re.fullmatch(pattern, piece):
                    ...
                    time_html = f'<div class="timeline-time">{safe_inline(buf_time)}</div>' if buf_time else ""
                    items_html.append(
                        f'<div class="timeline-item"><span class="timeline-dot"></span>'
                        f'{time_html}<div class="timeline-activity">{safe_inline(piece)}</div></div>'
                    )
                    buf_time = ""
        else:
            items_html.append(
                f'<div class="timeline-item"><span class="timeline-dot"></span>'
                f'<div class="timeline-activity">{safe_inline(text)}</div></div>'
            )
...
def render_assistant_entry(entry, idx):
    ...
    parts.append(f'<div class="bubble assistant">{safe_inline(entry["content"])}</div>')
    ...
    if structured:
        ...
        if restaurants:
            ...
            parts.append(f'<div class="rec-top-name">{safe_inline(top["name"])}</div>')
            if top["desc"]:
                parts.append(f'<div class="rec-top-desc">{safe_inline(top["desc"])}</div>')
            parts.append('</a>')
            if rest:
                parts.append('<div class="rec-grid">')
                for r in rest:
                    parts.append(f'<a class="rec-card" href="{r["link"]}" target="_blank" rel="noopener">')
                    parts.append(f'<div class="rec-card-name">{safe_inline(r["name"])}</div>')
                    if r["desc"]:
                        parts.append(f'<div class="rec-card-desc">{safe_inline(r["desc"])}</div>')
                    parts.append('</a>')
                parts.append('</div>')
            parts.append('</div></div>')

        if plan:
            parts.append('<div class="assistant-extra"><div class="plan-wrap">')
            parts.append('<div class="plan-card">')
            parts.append('<div class="plan-heading">✦ Your Date Plan</div>')
            timeline = plan.get("timeline")
            if timeline:
                parts.append(
                    f'<div class="plan-block"><div class="plan-label">'
                    f'{PLAN_ICONS["timeline"]} {PLAN_LABELS["timeline"]}</div>'
            ...
            for key, label in PLAN_FIELDS:
                ...
                if val:
                    ...
                    parts.append(
                        f'<div class="plan-block"><div class="plan-label">{icon} {label}</div>'
                        f'<div class="plan-value">{safe_inline(val)}</div></div>'
                    )
            parts.append('</div>')
            parts.append('</div></div>')

    # Hidden element holding the plain-text copy payload. Reading it by id
    # (rather than stuffing the whole thing into a data-* attribute) sidesteps
    # any attribute-escaping edge cases for long, multi-paragraph text.
    copy_text = build_assistant_copy_text(entry)
    copy_id = f"aisle-copy-src-{idx}"
    parts.append(
        f'<span id="{copy_id}" style="display:none;">{safe_inline(copy_text)}</span>'
        f'<div class="assistant-extra"><div class="copy-row assistant">'
        f'<button class="copy-btn" data-copy-target="{copy_id}">⧉ Copy</button></div></div>'
    )
...
for idx, entry in enumerate(st.session_state.chat_log):
    if entry["role"] == "user":
        copy_id = f"aisle-copy-src-user-{idx}"
        st.markdown(
            f'<div class="msg-row user"><div>'
            f'<div class="bubble-label" style="text-align:right;">You</div>'
            f'<div class="bubble user">{safe_inline(entry["content"])}</div>'
            f'<span id="{copy_id}" style="display:none;">{safe_inline(entry["content"])}</span>'
            f'<div class="copy-row user"><button class="copy-btn" data-copy-target="{copy_id}">⧉ Copy</button></div>'
            f'</div></div>',
            unsafe_allow_html=True,
        )
    else:
        render_assistant_entry(entry, idx)
...
if st.session_state.awaiting_location:
    st.markdown(
        '''<div class="approval-card">
              ...
              Use your network location, or just type your city in the chat box below.</div>
            </div>''',
        unsafe_allow_html=True,
    )
    loc_c1, loc_c2 = st.columns([1, 3])
    with loc_c1:
        if st.button("✦ Use my location", key="aisle_geo_btn", use_container_width=True):
            with st.spinner("Locating…"):
                detected_city = get_ip_city()
            st.session_state.awaiting_location = False
            if detected_city:
                ...
                st.session_state.chat_log.append({
                    "role": "assistant",
                    "content": "I couldn't detect your location automatically — mind typing your city instead?",
                    "structured": None,
                })
            st.rerun()
    with loc_c2:
        st.markdown(
            '<div class="dossier-sub" style="padding-top:0.55rem;">Uses your network\'s approximate '
            'city — not exact GPS.</div>',
            unsafe_allow_html=True,
        )
